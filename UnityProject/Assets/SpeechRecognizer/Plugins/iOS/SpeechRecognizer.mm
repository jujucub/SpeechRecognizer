...
- (void)startListening {
    ...
    [audioSession setMode:AVAudioSessionModeDefault error:&outError];
    [audioSession setActive:true error:&outError];
    if(outError != nil){
        if(self.errorCallback != nil){
            const char* domain = MakeStringCopy([outError.domain UTF8String]);
            self.errorCallback(outError.code, domain);
        }
        return;
    }
